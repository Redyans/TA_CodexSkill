...
> **语言与编码**：中文，UTF-8
> **模块入口**：本文件是场景与角色 Shader 规则的唯一规范来源；TA 规则总入口见 [../README_Tech_TADevelopmentRules.md](../README_Tech_TADevelopmentRules.md)，配套报告模板见 [references/shader-variant-report.md](references/shader-variant-report.md)，变体裁剪与构建安全参考见 [references/shader-variant-stripping-and-build-safety.md](references/shader-variant-stripping-and-build-safety.md)，材质导入与 SH 求值参考见 [references/urp-material-import-and-sh-evaluation.md](references/urp-material-import-and-sh-evaluation.md)，KeywordEnum 功能开关与材质关键字同步参考见 [references/keyword-enum-feature-switch-and-gui-sync.md](references/keyword-enum-feature-switch-and-gui-sync.md)。ProjectACG 当前变体事实见 [ProjectACG Shader 变体裁剪 Profile](Profiles/ProjectACG/shader-variant-stripping.md)，BattleSceneBaseLit 专用事实见 [BattleSceneBaseLit 变体剥离 Profile](Profiles/ProjectACG/battle-scene-baselit-variant-stripping.md)，材质导入与 SH 求值事实见 [ProjectACG URP 材质导入与 SH 求值 Profile](Profiles/ProjectACG/urp-material-import-and-sh-evaluation.md)，顶点色混合层数开关事实见 [ProjectACG 顶点色混合层数开关 Profile](Profiles/ProjectACG/vertex-blend-layer-count-keyword-switch.md)。
...
当前 ProjectACG `Valkyria/Scene/VertexBlend4` 的 `_BlendLayerCount` 层数下拉、`_BLENDLAYERCOUNT_LAYER1` ~ `_BLENDLAYERCOUNT_LAYER4` 关键字、五个 Pass 的声明、存量材质迁移，以及「下拉为 Layer1 却渲染 2 层」的现象、根因和验证边界见 [ProjectACG 顶点色混合层数开关 Profile](Profiles/ProjectACG/vertex-blend-layer-count-keyword-switch.md)；跨项目的 KeywordEnum 开关写法、变体计数与 ShaderGUI 关键字同步方法见 [`references/keyword-enum-feature-switch-and-gui-sync.md`](references/keyword-enum-feature-switch-and-gui-sync.md)。
...
- **角色/怪物溶解、空间采样、阈值边界、骨骼传值或预览丢材质排查**：读取 `CMP-01`、`CHR-01`、`CHR-04`、`CHR-05` 与 [`references/dissolve-space-threshold-and-material-lifecycle.md`](references/dissolve-space-threshold-and-material-lifecycle.md)，冻结阈值、空间、材质所有权和编辑器生命周期；当前 ProjectACG 再读取 [`Profiles/ProjectACG/character-death-dissolve.md`](Profiles/ProjectACG/character-death-dissolve.md)，以 `CharacterDeath`、Chara_V2 Bindings、Undo/Animation Mode 预览租约和当前验证状态为准。
...
- **NBShader 接入 GammaUI、2D UI 排序或 GameView/SceneView 差异**：读取 [`references/nbshader-2d-ui-gammaui-integration.md`](references/nbshader-2d-ui-gammaui-integration.md) 与通用 Gamma UI 参考，再核对目标工程的 Shader Pass、材质模式协议、RendererFeature、Canvas、相机、Layer、RT 和变体；不要只添加 `LightMode` 或把 SceneView fallback 当成像素一致证明。
...
- **材质 Inspector、Drawer、Keyword 与动画接口**：读取 `PRJ-02`、`Assets/Plugins/CustomShaderGUI/README_Tech_ShaderGUIFeatureShowcase.md`、`ShaderGUIFeatureShowcase.shader`、`Editor/SimpleShaderGUI.cs` 和对应 `PropertyDraw/*.cs`。默认以 `Scarecrow.SimpleShaderGUI` 的真实 Drawer、隐藏状态属性与 Keyword 行为为准，不引用外部 LWGUI/DDGUI 语法。用 `[KeywordEnum]` 做编译期档位裁剪、或出现「下拉显示与渲染不一致」时，追加读取 [`references/keyword-enum-feature-switch-and-gui-sync.md`](references/keyword-enum-feature-switch-and-gui-sync.md)；ProjectACG 顶点色混合层数开关的关键字、Pass 与存量材质事实见 [`Profiles/ProjectACG/vertex-blend-layer-count-keyword-switch.md`](Profiles/ProjectACG/vertex-blend-layer-count-keyword-switch.md)。
...
- **加密代理 Shader 明文还原与独立化**：先读 [`references/encrypted-proxy-shader-restoration.md`](references/encrypted-proxy-shader-restoration.md)，确认授权、代理/容器/工具映射、可信反序列化、Property/Keyword/Pass 保真、材质回退和 Unity 编译/视觉验证边界；批量提取与批量材质迁移必须分阶段。恢复源码含 `CustomEditor`、需要还原 Inspector/Keyword 联动、评估精确 Shader 名分支或迁移 ShaderGUI 时，继续读 [`references/shader-decryption-recovery-and-shadergui-migration.md`](references/shader-decryption-recovery-and-shadergui-migration.md)，并把跨工程迁移做成“Shader + GUI + 示例材质 + `.meta` GUID 预检”的完整闭环。
...
| 分支 / Keyword / GUI | `CTL-*`、`PRJ-02`、[`references/keyword-enum-feature-switch-and-gui-sync.md`](references/keyword-enum-feature-switch-and-gui-sync.md)、材质、脚本、动画、构建证据 | 评估变体、序列化和运行时写入；核对关键字列表首项是否等于默认档、各 Pass 是否一致。 |
...
| 解密 Shader 的 ShaderGUI 还原 / 跨工程迁移 | [`references/shader-decryption-recovery-and-shadergui-migration.md`](references/shader-decryption-recovery-and-shadergui-migration.md)、解密原文、原 `CustomEditor` 类型、材质、Editor 程序集和目标工程依赖 | 查清精确 Shader 名分支、Property/Mode/Keyword/顶点流映射，恢复 Inspector 与 Blend/Queue/Keyword 联动，并验证最小迁移文件闭包。 |
